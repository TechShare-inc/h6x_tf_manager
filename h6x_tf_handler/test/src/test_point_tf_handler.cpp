...
#include <h6x_tf_handler/point_tf_handler.hpp>
...
class TestPointTfNode : public rclcpp::Node
{
public:
  explicit TestPointTfNode(const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
  : Node("test_point_tf_node", options)
  {}
};

class TestPointTfHandler : public ::testing::Test
{
protected:
  TestPointTfNode::SharedPtr host_node;
  PointTfHandler::SharedPtr handler;
  ...
  virtual void SetUp()
  {
    this->host_node = std::make_shared<TestPointTfHandler>();
    this->executor.add_node(this->host_node);

    this->handler = std::make_shared<PointTfHandler>(
...
TEST_F(TestPointTfHandler, woSrcFrame)
{
...
TEST_F(TestPointTfHandler, woDistFrame)
{
  this->handler->configure();
  this->handler->setSrcFrameId(this->src_frame_id);
  // this->handler->setDistFrameId(this->dist_frame_id);
...
TEST_F(TestPointTfHandler, testActivationOK)
{
  this->handler->configure();
  this->setFrames();
...
TEST_F(TestPointTfHandler, testActivationNG)
{
  this->handler->configure();
...
TEST_F(TestPointTfHandler, testTransform1)
{
  this->handler->configure();
  this->setFrames();
  ...
  ASSERT_DOUBLE_EQ(out.point.x, 1.0);
}

TEST_F(TestPointTfHandler, testTransform2) {
  this->handler->configure();
  this->setFrames();
  using namespace std::chrono_literals;  // NOLINT
  this->handler->activate(100ms);
  ...
  ASSERT_EQ(out.header.frame_id, this->dist_frame_id);
  ASSERT_DOUBLE_EQ(out.point.x, 3.0);
}
